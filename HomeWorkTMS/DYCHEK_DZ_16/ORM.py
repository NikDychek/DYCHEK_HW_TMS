# С использованием ORM (Object-Relational Mapping):
...
engine = create_engine('sqlite:///HomeWork_16.db')
...
# Получение клиента из заказа (ORM)
for order in session.query(Order):
    client = order.client
    print(f"Заказ {order.name} принадлежит клиенту {client.name}")

# Получение списка заказов клиента (ORM)
for client in session.query(Client):
    print(f"Заказы клиента {client.name}:")
